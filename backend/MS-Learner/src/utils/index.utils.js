...
const SubscribeMessages = async (channel, service) => {
  ...
  //!Create queue
  const q = await channel.assertQueue("LearnerQueue");

  //!Bind Queue
  // This consumer will concume messages from Learner Only
  channel.bindQueue(q.queue, EXCHANGE_NAME, LEARNER_ROUTING_KEY);
